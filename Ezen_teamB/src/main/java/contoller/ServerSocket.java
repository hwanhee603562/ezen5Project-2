...
public class ServerSocket {
		...
		@OnOpen		// 매개변수 : 1. 접속된 클라이언트소켓(세션) 2. 접속된 회원아이디 3. 접속한 채팅방번호
			...
			MsgDto dto = null;
					...
					for(ClientDto clientDto : clientList) {
						if(clientDto.getSession() == session) {
							...
							boolean result = ChattingDao.getInstance().recordMsg(cmno, rmno, modMsg,
									clientDto.getIno(), clientDto.getRno());
							dto = new MsgDto(clientDto.getCmid(), modMsg);
							break;
						}
					}
